...
class TestAlbumModelIntegration(TestCase):
    ...
    def test_create_album(self):
        ...
        self.assertEqual(album.get_pictures(), [])
        self.assertEqual(len(album.get_albums()), 2)

    def test_sanitize_path__remove_first_slash(self):
        path = "/album"
        sanitized_path = Album.sanitize_path(path)
        self.assertEqual(sanitized_path, 'album')
    ...
    def test_sanitize_path__remove_two_slashes(self):
        path = "album1//album2"
        sanitized_path = Album.sanitize_path(path)
        self.assertEqual(sanitized_path, 'album1/album2')

    def test_sanitize_path__potentially_attack1(self):
        path = "/etc/"
        sanitized_path = Album.sanitize_path(path)
        self.assertEqual(sanitized_path, 'etc/')
